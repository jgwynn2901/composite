...
namespace FnsComposite
{
  ...
  public class Composite : CompositeBase, IEnumerable, IComposite, ISerializable
  {
    ...
    /// <summary>
    /// Every node can have childrent that may or may not be nodes themselves
    /// </summary>
    private Hashtable _objectList;
    /// <summary>
    /// Touch indicator 
    /// </summary>
    private bool _hasAddedSinceLastCommit;
    /// <summary>
    /// Delimit the xpath/callObject naming of attribute/elements
    /// </summary>
    static protected readonly string Delimiter = @"/\:"; // BUGBUG: move to configuration
    ...
    /// </value>
    virtual protected bool IsRoot { get { return Name == "CALL"; }}
    ...
    /// </summary>
    [CLSCompliant(false)]
    protected const UInt32 AttributeType = 32771;

    /// <summary>
    /// Binary serialization constant
    /// </summary>
    [CLSCompliant(false)]
    protected const UInt32 ObjectType = 32769;

    /// <summary>
    /// Binary serialization constant
    /// </summary>
    [CLSCompliant(false)]
    protected const UInt32 HeaderIndicator = 65535;

    /// <summary>
    /// Binary serialization constant
    /// </summary>
    [CLSCompliant(false)]
    protected const UInt16 LongTagIndicator = 32767;

    /// <summary>
    /// Gets the object list.
    /// </summary>
    protected Hashtable ObjectList
    {
    ...
    /// </value>
    protected bool HasAddedSinceLastCommit
    {
      get { return _hasAddedSinceLastCommit; }
      set { _hasAddedSinceLastCommit = value; }
    }
    ...
    public struct CompositeEnumerator : IEnumerator
    {
      ...
      public CompositeEnumerator(CompositeBase aNode)
      {
        ...
        var enumList = ((Composite)aNode)._objectList.GetEnumerator();
      ...
      public object Current
      {
        get
        {
          return _currentIndex < _arrayList.Count ? _arrayList[_currentIndex] : null;
        }
      }
      /// <summary>
      ...
      : base(name)
    {
      ...
      _hasAddedSinceLastCommit = false;
    }
    /// <summary>
    /// Initializes a new instance of the <see cref="Composite"/> class.
    /// </summary>
      ...
      : base(source.Name)
    {
      ...
      foreach (CompositeBase node in source)
      {
        ...
        if (newNode != null)
          _objectList.Add(newNode.Name, newNode);
    ...
    protected Composite(SerializationInfo info, StreamingContext context)
      : this(ClassName)
    {
    ...
    public override void Serialize(BinaryReader reader)
    {
      ...
      for (var i = 0; i < childCount; ++i)
      {
        ...
        var shortTag = (UInt16)reader.ReadInt16();
        UInt32 objectTag;

        if (shortTag == LongTagIndicator)
          objectTag = (UInt32)reader.ReadInt32();
        else
          objectTag = shortTag;
    ...
    public override void Serialize(BinaryWriter writer)
    {
      ...
      WordReader.WriteCount(writer, _objectList.Count);
      foreach (CompositeBase node in _objectList.Values)
      {
        // key
        CStringConverter.Write(writer, node.Name);
        if (node.IsLeaf)
          writer.Write((UInt16)AttributeType);
        else
      ...
      while (null != aChildNode)
      {
        switch (aChildNode.NodeType)
        {
          ...
          case XmlNodeType.Element:
            ...
            {
              var newNode =
                  CompositeFactory.CreateInstance(DecodeName(aChildNode.Name), aChildNode, String.Format("[{0}]", GetIndex(aChildNode)));
              Add(newNode);
    ...
    protected string ObjectName
    {
      get
      {
        return IsIndexed ? m_name.Substring(0, m_name.IndexOf('[')) : m_name;
      }
    }
    ...
    public bool Add(string name, string value, string type)
    {
      ...
      for (var i = 0; i < len; ++i)
      {
        ...
        if (null == (oNode2 = (Composite)node.GetNode(split[i])))
        {
          ...
          oNode2 = CompositeFactory.CreateInstance(split[i]);
          node.Add(oNode2);
        }
        node = oNode2;
      }
    ...
    public override bool HasValidNode(string name)
    {
      ...
      for (var i = 0; i < len; ++i)
      {
        ...
        {
          if (name == node.Name)
            return node._objectList.Count > 0;
    ...
    /// <returns></returns>
    virtual public bool Add(Composite oValue)
    {
      const bool results = true;
      //;

      if (_objectList.ContainsKey(oValue.Name))
      {
        var node = _objectList[oValue.Name] as Composite;
        if (node != null)
        {
      ...
      else
      {
        lock (_objectList.SyncRoot)
        {
          ...
          oValue.Parent = this;
          _objectList.Add(oValue.Name, oValue);
          _hasAddedSinceLastCommit = true;
        }
      }
      return results;
    }
    ...
    public void CopyChildren(Composite source)
    {
      foreach (CompositeBase comp in source)
      {
        ...
        else
        {
          ...
          if (_objectList.ContainsKey(comp.Name))
          {
            var child = _objectList[comp.Name] as Composite;
    ...
    public override string ToString()
    {
      ...
      var enumList = _objectList.GetEnumerator();
    ...
    protected CompositeBase GetNode(string name)
    {
      if (name == m_name)
        return this;
      var node = (CompositeBase)_objectList[name];
      return node;
    }
    ...
    {
      var results = _objectList.ContainsKey(name);
      if (results)
      {
        _objectList.Remove(name);
        _hasAddedSinceLastCommit = true;
      }
      return results;
    }
    ...
    protected bool AddAt(string name, string value, string type)
    {
      ...
      if (_objectList.ContainsKey(name))
      {
        var leafNode = _objectList[name] as CompositeLeaf;
        if (leafNode == null)
        {
    ...
    protected void AddLeafNode(CompositeLeaf node)
    {
      node.Depth = Depth + 1;
      node.Parent = this;
      _objectList.Add(node.Name, node);
      _hasAddedSinceLastCommit = true;
    }
    ...
    public override bool HasChanged
    {
      ...
      {
        // process the leaves
        var found = _hasAddedSinceLastCommit;
        if (!found)
        {
    ...
    /// <summary>
    /// Creates a composite based object directly from json 
    /// using CompositeFactory to manage conversion types.
    /// </summary>
    public static Composite CreateFromJson(string json)
    {
      Composite results = null;
      ...
      if (json.Length > 0)
      {
        using (JsonReader jsonReader = new JsonTextReader(new StringReader(json)))
        {
          ...
          jsonReader.Read(); // root PropertyName 

          results = CompositeFactory.CreateInstance(DecodeName(jsonReader.Value.ToString()));
          results.FromJson(jsonReader);
        }
      }
    ...
    private void ToJson(JsonWriter jsonWriter, int index )
    {
      ...
      if (arrayGroups.Count > 0)
      {
        foreach (var key in arrayGroups.Keys)
        {
          ...
          {
            jsonWriter.WriteStartObject();
            var child = (Composite)_objectList[string.Format("{0}[{1}]", key, i)];
    ...
    private void ProcessJsonArray(JsonReader jsonReader, string currentProperty)
    {
      ...
      while (notDone && jsonReader.Read())
      {
        switch (jsonReader.TokenType)
        {
          ...
          case JsonToken.StartObject:
            var nodeName = string.Format("{0}[{1}]", currentProperty, currentIndex++);
    ...
    public int? GetValueAsInt(string column)
    {
      ...
      if (int.TryParse(GetValue(column), out result))
        return result;
      return null;
    }
