...
namespace CompositeTests
{
	...
	public class CompositeTest
	{
		...
		/// </summary>
		[Test] [Ignore]public void TestCommit()
		{
		...
		/// <summary>
		/// NUNIT Tests the message object receive.
		/// </summary>
		[Test, Ignore]
		public void TestMessageObjectReceive()
		{
			IMessageObject message = new MessageObject {Destination = ".\\private$\\test"};
			var dateBefore = message.CreateDateTime;
			Assert.IsNotEmpty(dateBefore, "IMessage.CreateDateTime failed.");
			Console.WriteLine("Message Type: {0} Created on {1}", ((Composite)message).Name, message.CreateDateTime);
			Assert.IsTrue(message.Receive());
			var dateAfter = message.CreateDateTime;
			Assert.AreNotEqual(dateBefore, dateAfter, "Dates are the same!");
			Console.WriteLine(((Composite)message).ToXml());
		}
		/// <summary>
		/// Tests the get path.
